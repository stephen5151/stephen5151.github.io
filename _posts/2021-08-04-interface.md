...
## (7) 数字音频接口之光纤接口  
光纤接口（TOSLINK），器材上一般标为“*Optical*”。它的物理接口分为两种类型，一种是**标准方头**，另一种是在便携设备上常见的外观与3.5mm TRS接头类似的**圆头**。由于它是**以光脉冲的形式**来传输数字信号，因此单从技术角度来看，它是传输速度最快的.光纤同样采用S/PDIF接口输出，其*带宽高，信号衰减小*，常用于DVD播放器和AV功放，***支持PCM数字音频信号，Dolby以及DTS音频信号***。
